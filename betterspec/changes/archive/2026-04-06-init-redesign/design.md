...
Skills are already handled by the init command via agentskills.io format. Gemini CLI discovers them from `.gemini/skills/` or `.agents/skills/`. The adapter previously symlinked or copied the project `.agents/skills/` into `.agents/skills/` (now redundant) if the user used local skills.
