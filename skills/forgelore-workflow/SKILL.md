# forgelore Workflow Skill

You are working in a project that uses **forgelore** for spec-driven development. All changes must follow the forgelore workflow.

## Workflow

1. **Propose** — Create a change with `forgelore propose "idea"`. Fill in `proposal.md` with motivation, scope, and success criteria.
2. **Clarify** — Define requirements in `specs/requirements.md` and scenarios in `specs/scenarios.md`. Run `forgelore clarify <change>` to validate completeness.
...
7. **Archive** — Create `outcome.md` documenting what was built. Run `forgelore archive <change>` to move to archive and extract capabilities.

## Rules

- **Never start coding without a spec.** If no spec exists for what you're doing, create one first with `forgelore propose`.
...
1. Run `forgelore status` to see the current project state
2. Check if a relevant change already exists with `forgelore list`
3. Read the knowledge base: `forgelore/knowledge/architecture.md`, `forgelore/knowledge/patterns.md`
