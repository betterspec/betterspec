...
const DriftItemRow: React.FC<{ item: DriftItem }> = ({ item }) => (
  <InkBox paddingLeft={2}>
    <Text hex={SEVERITY_COLOR[item.severity] ?? colors.muted}>
...
const DiffDashboard: React.FC<DiffDashboardProps> = ({ projectRoot, changeName }) => {
  ...
  const scoreColor =
    report.score >= 80 ? colors.success : report.score >= 50 ? "#FFA500" : colors.error;

  return (
    <BetterspecBox
      title={changeName ? `Drift: ${changeName}` : "Drift Report"}
      borderColor={report.score >= 80 ? "success" : report.score >= 50 ? "warning" : "error"}
    >
      <InkBox flexDirection="column" gap={1}>
        {/* Score */}
        <InkBox>
          <Text dimColor>Score: </Text>
          <Text bold hex={scoreColor}>{report.score}/100</Text>
        ...
        {critical.length > 0 && (
          <InkBox flexDirection="column">
            <Text bold hex={SEVERITY_COLOR.critical}>Critical ({critical.length})</Text>
            {critical.map((item, i) => <DriftItemRow key={i} item={item} />)}
          </InkBox>
        )}
        ...
        {warnings.length > 0 && (
          <InkBox flexDirection="column">
            <Text bold hex={SEVERITY_COLOR.warning}>Warnings ({warnings.length})</Text>
            {warnings.map((item, i) => <DriftItemRow key={i} item={item} />)}
          </InkBox>
        )}
        ...
        {info.length > 0 && (
          <InkBox flexDirection="column">
            <Text hex={SEVERITY_COLOR.info}>Info ({info.length})</Text>
            {info.map((item, i) => <DriftItemRow key={i} item={item} />)}
          </InkBox>
        )}
      </InkBox>
    </BetterspecBox>
