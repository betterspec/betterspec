...
const ServerInfo: React.FC<{ port: number; projectRoot: string }> = ({
  ...
  <InkBox flexDirection="column">
    <Text>
      ...
      <Text color={colors.primary}>{port}</Text>
    </Text>
    <InkBox paddingTop={1} flexDirection="column">
      ...
      <InkBox flexDirection="column" paddingTop={0}>
        ...
        <Text>
          {"  "}Add to <Text color={colors.primary}>mcp.json</Text>:
        </Text>
        <Text dimColor>
      ...
      <InkBox flexDirection="column" paddingTop={1}>
        ...
        <Text>
          {"  "}
          <Text color={colors.primary}>npx @betterspec/mcp --cwd {projectRoot}</Text>
        </Text>
      </InkBox>
    </InkBox>
  </InkBox>
  ...
  child.on("error", (err) => {
    render(
      <BetterspecBox title="Server Error" borderColor="error">
        <Text color={colors.error}>Failed to start MCP server.</Text>
        <Text dimColor>
          Make sure <Text color={colors.primary}>@betterspec/mcp</Text> is installed:
        </Text>
        <Text>
          {"  "}
          <Text color={colors.primary}>npm install -g @betterspec/mcp</Text>
        </Text>
