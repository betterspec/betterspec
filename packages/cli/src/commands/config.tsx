  ...
  if (type === "not-found") {
    return (
      <BetterspecBox title="Not Found" borderColor="error">
        <Text>
          Key <Text hex={colors.primary}>{notFoundKey}</Text> not found in config.
        </Text>
      </BetterspecBox>
    );
  }
  ...
  if (type === "get" && typeof value === "string") {
    return (
      <BetterspecBox title="Config" borderColor="accent">
        <Text>
          <Text hex={colors.primary}>{key}</Text>
          <Text dimColor> = </Text>
          <Text>{value}</Text>
        </Text>
      </BetterspecBox>
  ...
  if (type === "set") {
    return (
      <BetterspecBox title="Config" borderColor="success">
        <Text hex={colors.success}>\u2713 Set </Text>
        <Text hex={colors.primary}>{key}</Text>
        <Text dimColor> = </Text>
    ...
    <BetterspecBox title="Configuration" borderColor="accent">
      <InkBox flexDirection="column">
        {Object.entries(obj ?? {}).map(([k, v]) => {
          ...
          return (
            <Text key={k}>
              <Text hex={colors.primary}>{k}</Text>
              <Text dimColor> = </Text>
              <Text>{String(v)}</Text>
            </Text>
          );
...
export async function configCommand(
  ...
  if (options?.list || (!key && !value)) {
    ...
    render(<ConfigOutput type="list" value={config} />);
    return;
  }
  ...
  if (key && !value) {
    ...
    if (val === undefined) {
      render(
        <ConfigOutput type="not-found" notFoundKey={key} />
      );
      process.exit(1);
    }

    if (typeof val === "object" && val !== null) {
      render(<ConfigOutput type="get" key={key} value={val as Record<string, unknown>} />);
    } else {
